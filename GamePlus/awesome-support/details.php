...
<div class="wpas wpas-ticket-details">
	...
	<table class="wpas-table wpas-ticket-replies tickets-history">
		<tbody>
			<tr valign="top">
				...
				<td>
					<div>
						<div class="ticket-username">
							<strong class="profilename"><?php echo apply_filters('wpas_fe_template_detail_author_display_name', $author->data->display_name, $post ); ?></strong>
						</div>
						<div class="ticket-time">
							<time class="timestamp" datetime="<?php echo get_the_date( 'Y-m-d\TH:i:s' ) . wpas_get_offset_html5(); ?>">
								...
								<span class="date-ago"><?php printf( __( '%s قبل', 'awesome-support' ), human_time_diff( get_the_time( 'U', $post->ID ), current_time( 'timestamp' ) ) ); ?></span>
							</time>
						</div>
					</div>
					 ...
					 */
					echo '<div class="reply-content">' .  make_clickable( apply_filters( 'the_content', $post->post_content ) ) . '</div>';
